...
/**
 * @mainpage
 * This is a simple-to-use text parsing library for loosely-formalized texts.
 *
 * The philosophy behind the API is based on the concept of Hidden Markov Models (HMMs)
 * (Note: Though HMMs are widely used in machine learning, this library does not involve the
 * "learning" part of the technique. Instead, the parameters of the model are specified
 * by the user)
 *
 * Imagine that your text has been generated by a certain stochastic process (or has been typed by
 * a monkey, whichever makes more sense). At every moment, the character that the monkey will type
 * is random and depends on what she's thinking. And once the character has been typed, the monkey's
 * next state-of-mind only depends on her current state-of-mind and the character typed. To use this
 * library, your job would be to describe the probabilities for the monkey to type any given character
 * in every possible state, and the probabilities to transition to each possible next state. The
 * Parser in this library will read your text and tell you what the monkey was thinking.
 *
 * The state is represented by the abstract class ChunkState, which has methods to return exactly
 * the kind of probabilities mentioned above. Yet, I hope you will not need to implement
 * its interface: the true power of the library comes from the numerous ways to combine simpler
 * descriptions of text "chunks" into more complex ones. See the
 * <a href="hmmparser_8h.html#func-members">free functions</a>
 */
...
/**
 * @internal A ChunkState which generates an empty string and returns a Chunk given to it at construction
 */
template<typename Chunk>
...
/**
 * @internal A ChunkState which generates an arbitrary character and returns that character as the generating Chunk
 */
...
/**
 * @internal Generates a sequence of random characters of random length; returns Void as the generating Chunk
 */
...
/**
 * @internal Generates variable-length sequence of white-space characters; returns Void as the generating Chunk
 */
...
/**
 * @internal A ChunkState which generates a fixed string, and returns that string as the generating Chunk
 */
...
/**
 * @internal A ChunkState which generates the same text as a given other ChunkState, but applies a transformation (function)
 * to the generator Chunk
 */
...
/**
 * @internal A ChunkState which generates one out of a list of chunks, given by their initial ChunkState's
 */
template<typename Chunk>
...
namespace continuation_impl
{
    ...
    /**
     * @internal The state of the continuation of a ChunkTBC where CurChunk will not be finished before the current character.
     */
    template<typename LastChunk, typename CurChunk>
